...
function fetchWeatherData(location) {
    ...
    fetch(apiUrl).then(response => response.json()).then(data => {
        ...
        todayWeatherIcon.className = `bx bx-${weatherIconMap[todayWeatherIconCode]}`; // from here is the problem
        ...
        const todayHumiditiy = `${data.list[0].main.humidity}%`;
        ...
        dayInfoContainer.innerHTML = `
            ...
            <div>
                ...
                <span class="value">${todayPercipitation}
                </span>
            </div>
            <div>
                <span class="title">HUMIDITY</span>
                <span class="value">${todayHumiditiy}
                </span>
            </div>
            <div>
        ...
        for (const dayData of nextDaysData) {
            ...
            // Ensure the day isnt duplicated with today
...
// fetach weather data on document load for default location 
